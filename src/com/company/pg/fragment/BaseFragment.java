...
public abstract class BaseFragment extends Fragment {
	...
	private XPGWifiSDKListener sdkListener = new XPGWifiSDKListener(){ 
		...
		{
			BaseFragment.this.didRegisterUser(error, errorMessage, uid, token); 
		}

		@Override
		public void didBindDevice(int error, String errorMessage, String did) {
			BaseFragment.this.didBindDevice(error, errorMessage, did);
		};
		...
		public void didChangeUserPassword(int error, String errorMessage) {
			BaseFragment.this.didChangeUserPassword(error, errorMessage);
		};

		@Override
		public void didRequestSendVerifyCode(int error, String errorMessage) {
			BaseFragment.this.didRequestSendVerifyCode(error, errorMessage);
		};
	...
	protected XPGWifiDeviceListener deviceListener = new XPGWifiDeviceListener() {
		...
		public void didLogin(XPGWifiDevice device, int result) {
			BaseFragment.this.didLogin(device, result);
		};

		@Override
		public void didDeviceOnline(XPGWifiDevice device, boolean isOnline) {
			BaseFragment.this.didDeviceOnline(device, isOnline);
		};
		...
		public void didQueryHardwareInfo(XPGWifiDevice device, int result, java.util.concurrent.ConcurrentHashMap<String,String> hardwareInfo) {
			BaseFragment.this.didQueryHardwareInfo(device, result, hardwareInfo);
		};
	};
	...
	public static XPGWifiDevice findDeviceByMac(String mac, String did) {
		...
		Log.i("count", BaseFragment.deviceslist.size() + "");
		for (int i = 0; i < BaseFragment.deviceslist.size(); i++) {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mCenter = MessageCenter.getInstance(BaseFragment.this.getActivity().getApplicationContext());
		setmanager = new SettingManager(BaseFragment.this.getActivity());
	}
